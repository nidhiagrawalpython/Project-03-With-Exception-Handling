...
public class FacultyModelJDBCImpl implements FacultyModelInt {
	...
	public long add(FacultyDTO dto) throws ApplicationException, DuplicateRecordException {
		long pk = 0;
		  ...
		  SubjectModelInt model2 =ModelFactory.getInstance().getSubjectModel(); SubjectDTO dto3 =
		 model2.findByPK(dto.getSubjectId()); 
		 String SubjectName =dto3.getName();
		 /* 
	...
	public FacultyDTO findByEmailId(String emailId) throws ApplicationException {
		...
		try {
			con = JDBCDataSource.getConnection();
			PreparedStatement ps = con.prepareStatement("select * from st_faculty where EMAIL=?");
	...
	public void update(FacultyDTO dto) throws DatabaseException, ApplicationException  {
		...
		try {
			...
			ps.setString(5, dto.getMobileNo());
			ps.setString(6, dto.getEmail());
			ps.setLong(7, dto.getCollegeId());
			ps.setString(8, dto.getCollegeName());
			ps.setLong(9, dto.getCourseId());
			ps.setString(10, dto.getCourseName());
			ps.setLong(11, dto.getSubjectId());
			ps.setString(12, dto.getSubjectName());
			ps.setString(13, dto.getCreatedBy());
			ps.setString(14, dto.getModifiedBy());
			ps.setTimestamp(15, dto.getCreatedDatetime());
			ps.setTimestamp(16, dto.getModifiedDatetime());
	...
	public List search(FacultyDTO dto1, int pageNo, int pageSize) throws ApplicationException {
		...
		if (dto1 != null) {
			...
			if ((dto1.getEmail() != null) && (dto1.getEmail().length() > 0)) {
				sql.append(" AND EMAIL like '" + dto1.getEmail() + "%'");
			}
